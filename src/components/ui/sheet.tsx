  ...
  const hasSheetTitle = React.Children.toArray(children).some((child) => {
    if (React.isValidElement(child)) {
      return child.type === SheetTitle || 
             (child.props && child.props['data-slot'] === 'sheet-title') ||
             (typeof child.type === 'function' && child.type.name === 'SheetTitle');
    }
    return false;
  });

  // Check if children contains a SheetDescription
  const hasSheetDescription = React.Children.toArray(children).some((child) => {
    if (React.isValidElement(child)) {
      return child.type === SheetDescription || 
             (child.props && child.props['data-slot'] === 'sheet-description') ||
             (typeof child.type === 'function' && child.type.name === 'SheetDescription');
    }
    return false;
  });
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        aria-describedby={!hasSheetDescription ? undefined : undefined}
        {...props}
      >
        {!hasSheetTitle && (
          <VisuallyHidden>
